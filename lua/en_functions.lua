...
function EnhancedDamage.DropWeapon(ply,chance,attacker)
   ...
   if (chnc > (100 - chance) then
          if (ply:IsPlayer()) then
             if (table.HasValue(
                    weaponblacklist, ply:GetActiveWeapon():GetClass())) then
             return end --Exclude the blacklist stuff
             ...
             if GetConVar("enhanceddamage_npcweapondrop"):GetBool() then
                ...
                if IsValid(weapon) then
                   ...
                   for _, v in pairs(EnhancedDamage.EntTable) do
                      if (v:IsPlayer()) then
                         -- Attempt to make the npc run away from players
                         ply:AddEntityRelationship(v, 2, 99)
                      end
                   end
                end
             end
